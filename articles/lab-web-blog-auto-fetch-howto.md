---
title: 物联网实验室站点如何从 UEM-Stu-Blog 自动拉取博客
...
# 物联网实验室站点如何从 UEM-Stu-Blog 自动拉取博客

UEM-Stu 组织里，博客文章统一存放在 [UEM-Stu-Blog](https://github.com/UEM-Stu/UEM-Stu-Blog) 内容仓库；主站 [UEM-Stu-Site](https://github.com/UEM-Stu/UEM-Stu-Site) 在 build 时拉取全部文章展示。各实验室还有独立门户站（如 [IoT-lab-web](https://github.com/UEM-Stu/IOT-lab-web)），只需要展示**本实验室**的文章。
...
| 仓库 | 角色 |
|------|------|
| **UEM-Stu-Blog** | 唯一内容源，Markdown + 图片 |
| **UEM-Stu-Site** | 主站，展示全部文章（Vite） |
...
2. 否则 → 浅克隆 `UEM-Stu-Blog` 到 `content/blog/`
...
投稿时在 UEM-Stu-Blog 的文章顶部标注所属实验室：
...
主站 UEM-Stu-Site **不过滤**，继续展示全部；各实验室站只改 `labCode` 即可复用同一套 Blog 内容。
...
站点部署在 `https://uem-stu.github.io/IOT-lab-web/`，Next.js 的 `basePath` 为 `/IOT-lab-web`。`<Link>` 和 CSS 会自动加前缀，但 **react-markdown 生成的 `<img src>` 不会**，必须手动加：
...
- run: npm run build
  env:
    GITHUB_PAGES: "true"
    NEXT_PUBLIC_SITE_URL: https://uem-stu.github.io/IOT-lab-web
```
...
1. 在 **UEM-Stu-Blog** 写稿，frontmatter 加 `labs: [IoT-Lab]`
2. PR 合并后 Blog 仓库更新
...
若你在组织内维护其他实验室站点，欢迎复用这套模式，并在 UEM-Stu-Blog 投稿时记得标注 `labs` 字段。
